...
function Message({ selectedChatData, message, downloadFile }) {
	const { setIsDownloading, setFileDownloadProgress } = useAppStore();
...
function ChannelMessage({ selectedChatData, message, downloadFile }) {
					...
					<Avatar className="h-8 w-8 rounded-full overflow-hidden">
						{message.sender.image ? (
							<div>
								...
								<AvatarFallback
									className={`uppercase h-8 w-8 text-lg flex items-center justify-center rounded-full bg-[purple] text-purple-900`}
								>
		...
		try {
			const response = await apiClient.get(`${HOST}/${filePath}`, {
				...
				onDownloadProgress: (data) => {
					...
					setFileDownloadProgress(progressCompleted);
				},
			});
